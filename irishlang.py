...
class irishspell:
    def __init__(self):
        self.file1 = open("irish.txt", "r", encoding = "UTF-16")
        self.corpus1 = self.file1.read()
        self.frequency_dictionary = ast.literal_eval(self.corpus1)
        self.file1.close()
        self.file3 = open("frequency.txt", "r", encoding = "UTF-16")
        self.corpus3 = self.file3.read()
        self.frequency = ast.literal_eval(self.corpus3)
        self.file3.close()
        self.suggestions = {}
        self.probability = 25
    ...
    def spell(self, word):
        ...
        for word1 in self.frequency_dictionary:
            self.probabilities[word1] = math.pow(self.damerau_levenshtein_distance(word, word1), self.probability) 
